...
// M5GFX compatibility macros
#define getViewportWidth()    width()
#define getViewportHeight()   height()
#define setViewport(x,y,w,h)  setCursor(0,0)
#define resetViewport()       setCursor(0,0)
...
#define MSG_VERSION                         "v0.83"                 // the version displayed on the splash screen at the lower right; β
  ...
  #define DEFAULT_TEXT_PADDING              20                      // necessary? perhaps just use a formula throughout the code? e.g. (tft.fontHeight() / 3)
#endif
...
const char msg_select_beacon_type[]     = "";
const char msg_select_beacon[]          = "";
...
// TFT_eSPI requires special setup with LilyGo AMOLED devices
 ...
 #define tft M5Cardputer.Display    // TFT_eSPI display interface
...
*/
        ...
        tft.setTextColor(BEACON_CONTROL_COUNTDOWN_COLOR, TFT_BLACK);
...
void button_handler() {
  ...
  static uint32_t key_enter_down_time = 0;
  static uint32_t key_bs_down_time = 0;
  static bool key_enter_held = false;
  static bool key_bs_held = false;

  if (M5Cardputer.Keyboard.isChange()) {
    if (M5Cardputer.Keyboard.isPressed()) {
      Keyboard_Class::KeysState status = M5Cardputer.Keyboard.keysState();

      // up/down navigation — instant, no long press needed
      for (auto key : status.word) {
        if (key == ';') {
          button2(SHORT_PRESS);    // navigate up
          last_activity = millis();
        }
        if (key == ',') {
          button2(LONG_PRESS);     // navigate down
          last_activity = millis();
        }
      }

      // Enter and Backspace — track for short/long press
      if (status.enter && !key_enter_held) {
        key_enter_down_time = millis();
        key_enter_held = true;
      }
      if (status.del && !key_bs_held) {
        key_bs_down_time = millis();
        key_bs_held = true;
      }
      ...
      // key released
      if (key_enter_held) {
        if (millis() - key_enter_down_time > SHORT_PRESS_TIME) {
          button1(LONG_PRESS);     // hold Enter = back
        } else {
          button1(SHORT_PRESS);    // tap Enter = select
        }
        key_enter_held = false;
        last_activity = millis();
      }
      if (key_bs_held) {
        if (millis() - key_bs_down_time > SHORT_PRESS_TIME) {
          button2(LONG_PRESS);
        } else {
          button2(SHORT_PRESS);    // tap Backspace = also navigate
        }
        key_bs_held = false;
        last_activity = millis();
      }
